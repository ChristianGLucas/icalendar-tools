...
final class ICal4jHelper {
    ...
    /**
     * Emits a RECURRENCE-ID property (RFC 5545 §3.8.4.4) with its TZID and RANGE.
     *
     * <p>RANGE is emitted ONLY when it is THISANDFUTURE — the sole value this revision
     * of iCalendar defines. RFC 5545 §3.2.13 deprecates THISANDPRIOR and says it "MUST
     * NOT be generated by applications", so an inherited THISANDPRIOR (or any other
     * junk value) is DROPPED rather than written out.
     *
     * <p>Dropped, not rejected. Failing here would kill the whole calendar over one
     * legacy component, which contradicts this package's stated posture that individual
     * malformed components are preserved as-is rather than rejecting the document — and
     * a legacy RFC 2445 file that ParseICalendar reads happily would become impossible
     * to rebuild. Dropping is also semantically safe: RFC 5545 §3.2.13 makes an ABSENT
     * RANGE mean "the single instance specified by the recurrence identifier", which is
     * exactly how ExpandOccurrences already treats THISANDPRIOR on the read side. So
     * the two agree instead of one accepting what the other refuses.
     */
    private static void addRecurrenceId(PropertyList<Property> props, String value, String tzid,
                                        String range) throws IcalException {
        if (value.isEmpty()) return;
        RecurrenceId p = new RecurrenceId();
        setDateProperty(p, value, tzid);
        if ("THISANDFUTURE".equalsIgnoreCase(range)) {
            p.getParameters().add(new Range("THISANDFUTURE"));
        }
        props.add(p);
    }
    ...
    static VEvent buildEvent(ICalEvent e) throws IcalException {
        try {
            ...
            addRecurrenceId(props, e.getRecurrenceId(), e.getRecurrenceIdTzid(), e.getRecurrenceIdRange());
    ...
    static ICalTodo todoOf(VToDo t) {
        ICalTodo.Builder b = ICalTodo.newBuilder()
                ...
                .setRecurrenceId(dateValueOf(t, Property.RECURRENCE_ID))
                .setRecurrenceIdTzid(dateTzidOf(t, Property.RECURRENCE_ID))
                .setRecurrenceIdRange(paramValue(firstProperty(t, Property.RECURRENCE_ID), Parameter.RANGE));
    ...
    static VToDo buildTodo(ICalTodo t) throws IcalException {
        try {
            ...
            addRecurrenceId(props, t.getRecurrenceId(), t.getRecurrenceIdTzid(), t.getRecurrenceIdRange());
    ...
    static ICalJournal journalOf(VJournal j) {
        ...
        ICalJournal.Builder b = ICalJournal.newBuilder()
                ...
                .setRecurrenceId(dateValueOf(j, Property.RECURRENCE_ID))
                .setRecurrenceIdTzid(dateTzidOf(j, Property.RECURRENCE_ID))
                .setRecurrenceIdRange(paramValue(firstProperty(j, Property.RECURRENCE_ID), Parameter.RANGE));
    ...
    static VJournal buildJournal(ICalJournal j) throws IcalException {
        try {
            ...
            addRecurrenceId(props, j.getRecurrenceId(), j.getRecurrenceIdTzid(), j.getRecurrenceIdRange());
