...
final class ICal4jHelper {
    ...
    static VEvent buildEvent(ICalEvent e) throws IcalException {
        try {
            ...
            if (!e.getRecurrenceId().isEmpty()) {
                // A component cannot be both a recurring master and an override of one.
                // RECURRENCE-ID is load-bearing for occurrence suppression, so emitting
                // this pair would produce a document whose every expanded instance
                // claims to replace the same single instant — and could suppress a real
                // master occurrence elsewhere. Reject it rather than emit it.
                if (!e.getRrule().isEmpty()) {
                    throw new IcalException("INVALID_ARGUMENT",
                            "VEVENT \"" + e.getUid() + "\" sets both rrule and recurrence_id: a "
                                    + "RECURRENCE-ID override replaces ONE instance of a series and "
                                    + "cannot itself define a recurrence rule");
                }
                RecurrenceId p = new RecurrenceId();
                setDateProperty(p, e.getRecurrenceId(), e.getRecurrenceIdTzid());
                String range = e.getRecurrenceIdRange();
                if (!range.isEmpty()) {
                    // RFC 5545 §3.2.13: THISANDFUTURE is the only value this revision
                    // defines; THISANDPRIOR is deprecated and "MUST NOT be generated by
                    // applications". Emitting one would make this node a source of
                    // non-conformant iCalendar.
                    if (!"THISANDFUTURE".equalsIgnoreCase(range)) {
                        throw new IcalException("INVALID_ARGUMENT",
                                "VEVENT \"" + e.getUid() + "\" has recurrence_id_range \"" + range
                                        + "\": the only RANGE value RFC 5545 allows an application to "
                                        + "generate is THISANDFUTURE (THISANDPRIOR is deprecated)");
                    }
    ...
    static VToDo buildTodo(ICalTodo t) throws IcalException {
        try {
            ...
            if (!t.getRecurrenceId().isEmpty()) {
                if (!t.getRrule().isEmpty()) {
                    throw new IcalException("INVALID_ARGUMENT",
                            "VTODO \"" + t.getUid() + "\" sets both rrule and recurrence_id: a "
                                    + "RECURRENCE-ID override replaces ONE instance of a series and "
                                    + "cannot itself define a recurrence rule");
                }
                RecurrenceId p = new RecurrenceId();
                setDateProperty(p, t.getRecurrenceId(), t.getRecurrenceIdTzid());
                String range = t.getRecurrenceIdRange();
                if (!range.isEmpty()) {
                    if (!"THISANDFUTURE".equalsIgnoreCase(range)) {
                        throw new IcalException("INVALID_ARGUMENT",
                                "VTODO \"" + t.getUid() + "\" has recurrence_id_range \"" + range
                                        + "\": the only RANGE value RFC 5545 allows an application to "
                                        + "generate is THISANDFUTURE (THISANDPRIOR is deprecated)");
                    }
                    p.getParameters().add(new Range("THISANDFUTURE"));
    ...
    static VJournal buildJournal(ICalJournal j) throws IcalException {
        try {
            ...
            if (!j.getRecurrenceId().isEmpty()) {
                RecurrenceId p = new RecurrenceId();
                setDateProperty(p, j.getRecurrenceId(), j.getRecurrenceIdTzid());
                String range = j.getRecurrenceIdRange();
                if (!range.isEmpty()) {
                    if (!"THISANDFUTURE".equalsIgnoreCase(range)) {
                        throw new IcalException("INVALID_ARGUMENT",
                                "VJOURNAL \"" + j.getUid() + "\" has recurrence_id_range \"" + range
                                        + "\": the only RANGE value RFC 5545 allows an application to "
                                        + "generate is THISANDFUTURE (THISANDPRIOR is deprecated)");
                    }
                    p.getParameters().add(new Range("THISANDFUTURE"));
