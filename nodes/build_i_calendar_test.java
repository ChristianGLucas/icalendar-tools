...
public class BuildICalendarTest {
    ...
    @Test
    public void testBuildRejectsDeprecatedThisAndPriorRange() {
        // RFC 5545 §3.2.13: "The value 'THISANDPRIOR' is deprecated by this revision of
        // iCalendar and MUST NOT be generated by applications." This node's contract is
        // to generate a VALID document, so it must refuse rather than emit one — and
        // ValidateICalendar does not catch RANGE, so nothing else would stop it.
        AxiomContext ax = new TestContext();
        ICalTextOutput built = BuildICalendar.buildICalendar(ax, ICalCalendar.newBuilder()
                .addEvents(ICalEvent.newBuilder()
                        .setUid("tap-1").setSummary("Bad range")
                        .setDtstart("20260811T090000Z").setDtend("20260811T100000Z")
                        .setRecurrenceId("20260811T090000Z")
                        .setRecurrenceIdRange("THISANDPRIOR"))
                .build());
        assertTrue(built.hasError(), "THISANDPRIOR must be refused, not emitted");
        assertEquals("INVALID_ARGUMENT", built.getError().getCode());
        assertTrue(built.getError().getMessage().contains("THISANDFUTURE"),
                "the error must name the accepted value; got: " + built.getError().getMessage());
        assertTrue(built.getError().getMessage().contains("recurrence_id_range"),
                "the error must name the FIELD at fault, not just the ical4j value; got: "
                        + built.getError().getMessage());

        // ...and the legal value still works, so the gate is not simply refusing everything.
    ...
    @Test
    public void testBuildRejectsAnEventThatIsBothAMasterAndAnOverride() {
        // recurrence_id is load-bearing for occurrence suppression, so a component that
        // carries BOTH an rrule and a recurrence_id is not merely odd — expanded, every
        // one of its instances claims to replace the same single instant, and it can
        // suppress a real master occurrence elsewhere in the calendar. A caller
        // assembling an ICalCalendar programmatically from ListEvents output could
        // produce this by accident, so it is refused at build time.
        AxiomContext ax = new TestContext();
        ICalTextOutput built = BuildICalendar.buildICalendar(ax, ICalCalendar.newBuilder()
                .addEvents(ICalEvent.newBuilder()
                        .setUid("f1").setSummary("Both")
                        .setDtstart("20260810T090000Z").setDtend("20260810T100000Z")
                        .setRrule("FREQ=DAILY;COUNT=3")
                        .setRecurrenceId("20260810T090000Z"))
                .build());
        assertTrue(built.hasError(), "an rrule + recurrence_id component must be refused");
        assertEquals("INVALID_ARGUMENT", built.getError().getCode());
        assertTrue(built.getError().getMessage().contains("recurrence_id")
                        && built.getError().getMessage().contains("rrule"),
                "the error must name both conflicting fields; got: " + built.getError().getMessage());
    }
}
