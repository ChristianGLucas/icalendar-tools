...
public class BuildICalendarTest {
    ...
    @Test
    public void testRoundTripPreservesRecurrenceIdOnVTodoToo() {
        // RFC 5545 §3.8.4.4 defines RECURRENCE-ID for VTODO exactly as for VEVENT, and
        // ICalTodo already models rrule/rdate/exdate — so recurring to-dos, and edits
        // to a single instance of one, are in scope. Dropping it here is the same
        // corruption as on VEVENT: one edited to-do silently becomes two.
        AxiomContext ax = new TestContext();
        String src = String.join("\r\n",
                "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Test//Test//EN",
                "BEGIN:VTODO", "UID:t-1@example.com", "DTSTAMP:20260801T000000Z",
                "DTSTART:20260810T090000Z", "DUE:20260810T170000Z",
                "RRULE:FREQ=DAILY;COUNT=3", "SUMMARY:Daily check", "END:VTODO",
                "BEGIN:VTODO", "UID:t-1@example.com", "RECURRENCE-ID:20260811T090000Z",
                "DTSTAMP:20260801T000000Z",
                "DTSTART:20260811T140000Z", "DUE:20260811T180000Z",
                "SUMMARY:Daily check (moved)", "END:VTODO",
                "END:VCALENDAR", "");
        ...
        assertEquals("", parsed.getTodos(0).getRecurrenceId(), "the master to-do has none");
        assertEquals("20260811T090000Z", parsed.getTodos(1).getRecurrenceId(),
                "the override to-do must report which instant it replaces");

        ICalTextOutput built = BuildICalendar.buildICalendar(ax, parsed);
        assertFalse(built.hasError(), "unexpected build error: " + built.getError());
        assertTrue(built.getIcsText().replace("\r\n ", "").contains("RECURRENCE-ID"),
                "the rebuilt .ics dropped VTODO's RECURRENCE-ID");
        ICalCalendar reparsed = ParseICalendar.parseICalendar(ax,
                ICalTextInput.newBuilder().setIcsText(built.getIcsText()).build());
        assertEquals("20260811T090000Z", reparsed.getTodos(1).getRecurrenceId(),
                "Parse(Build(x)) must round-trip RECURRENCE-ID on VTODO");
    }

    @Test
    public void testBuildDropsTheDeprecatedThisAndPriorRangeInsteadOfEmittingOrRejectingIt() {
        // RFC 5545 §3.2.13: "The value 'THISANDPRIOR' is deprecated by this revision of
        // iCalendar and MUST NOT be generated by applications." So it must not appear in
        // the output. But failing the whole calendar over one legacy component would
        // make a legacy RFC 2445 file that ParseICalendar reads happily impossible to
        // rebuild — and this package's stated posture is that individual malformed
        // components are preserved, not that they reject the document.
        //
        // Dropping the parameter is the RFC-conformant middle: an ABSENT RANGE means
        // "the single instance specified by the recurrence identifier", which is exactly
        // how ExpandOccurrences already treats THISANDPRIOR on the read side.
        AxiomContext ax = new TestContext();
        ICalTextOutput built = BuildICalendar.buildICalendar(ax, ICalCalendar.newBuilder()
                .addEvents(ICalEvent.newBuilder()
                        .setUid("tap-1").setSummary("Legacy range")
                        .setDtstart("20260811T090000Z").setDtend("20260811T100000Z")
                        .setRecurrenceId("20260811T090000Z")
                        .setRecurrenceIdRange("THISANDPRIOR"))
                .build());
        assertFalse(built.hasError(),
                "one legacy component must not fail the whole calendar: " + built.getError());
        String unfolded = built.getIcsText().replace("\r\n ", "");
        assertFalse(unfolded.contains("THISANDPRIOR"),
                "THISANDPRIOR MUST NOT be generated; it must be dropped: " + unfolded);
        assertTrue(unfolded.contains("RECURRENCE-ID"),
                "the RECURRENCE-ID itself must survive — only the RANGE parameter is dropped");
        assertFalse(unfolded.contains("RANGE="),
                "no RANGE parameter at all, rather than a substituted one");

        // ...and the one legal value is still emitted, so this is not a blanket strip.
        ICalTextOutput ok = BuildICalendar.buildICalendar(ax, ICalCalendar.newBuilder()
                .addEvents(ICalEvent.newBuilder()
                        .setUid("tap-2").setSummary("Good range")
                        .setDtstart("20260811T090000Z").setDtend("20260811T100000Z")
                        .setRecurrenceId("20260811T090000Z")
                        .setRecurrenceIdRange("THISANDFUTURE"))
                .build());
        assertFalse(ok.hasError(), "THISANDFUTURE must be accepted: " + ok.getError());
        assertTrue(ok.getIcsText().replace("\r\n ", "").contains("RANGE=THISANDFUTURE"));
    }

    @Test
    public void testLegacyThisAndPriorCalendarSurvivesAFullRoundTrip() {
        // The end-to-end version of the rule above: a real RFC 2445-era document must
        // go Parse -> Build -> Parse without the pipeline blowing up, and must still
        // expand to the same occurrences. Hand-computed truth for [10 Aug, 13 Aug):
        // the 11 Aug instance is replaced by the override at 14:00, so
        //   10 Aug 09:00 | 11 Aug 14:00 | 12 Aug 09:00
        AxiomContext ax = new TestContext();
        String legacy = String.join("\r\n",
                "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Legacy//EN",
                "BEGIN:VEVENT", "UID:leg-1", "DTSTAMP:20260801T000000Z",
                "DTSTART:20260810T090000Z", "DTEND:20260810T100000Z",
                "RRULE:FREQ=DAILY;COUNT=3", "SUMMARY:Standup", "END:VEVENT",
                "BEGIN:VEVENT", "UID:leg-1", "RECURRENCE-ID;RANGE=THISANDPRIOR:20260811T090000Z",
                "DTSTAMP:20260801T000000Z",
        ...
        ICalCalendar parsed = ParseICalendar.parseICalendar(ax,
                ICalTextInput.newBuilder().setIcsText(legacy).build());
        assertFalse(parsed.hasError(), "the legacy document must parse: " + parsed.getError());
        assertEquals("THISANDPRIOR", parsed.getEvents(1).getRecurrenceIdRange(),
                "ParseICalendar reports what the file actually said");

        ICalTextOutput built = BuildICalendar.buildICalendar(ax, parsed);
        assertFalse(built.hasError(), "the legacy document must rebuild: " + built.getError());

        gen.Messages.ICalOccurrenceList before = ExpandOccurrences.expandOccurrences(ax,
                gen.Messages.ICalExpandInput.newBuilder().setIcsText(legacy)
                        .setWindowStart("20260810T000000Z").setWindowEnd("20260813T000000Z").build());
        gen.Messages.ICalOccurrenceList after = ExpandOccurrences.expandOccurrences(ax,
                gen.Messages.ICalExpandInput.newBuilder().setIcsText(built.getIcsText())
                        .setWindowStart("20260810T000000Z").setWindowEnd("20260813T000000Z").build());
        List<String> starts = new java.util.ArrayList<>();
        for (gen.Messages.ICalEventOccurrence o : before.getOccurrencesList()) {
            starts.add(o.getOccurrenceStart());
        }
        assertEquals(List.of("20260810T090000Z", "20260811T140000Z", "20260812T090000Z"), starts,
                "THISANDPRIOR is treated as a plain single-instance override on the read side");
        assertEquals(before.getOccurrencesList(), after.getOccurrencesList(),
                "dropping the deprecated RANGE must not change what the calendar MEANS");
    }

    @Test
    public void testRfc5546RuleChangeShapeStillRoundTrips() {
        // An override carrying its own RRULE is how RFC 5546 clients express a
        // THISANDFUTURE *rule* change. This package does not implement those semantics
        // (documented limitation), but BuildICalendar must not REFUSE a shape
        // ParseICalendar accepts — that would make a real scheduling message
        // impossible to round-trip.
        AxiomContext ax = new TestContext();
        String src = String.join("\r\n",
                "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Test//EN",
                "BEGIN:VEVENT", "UID:rc-1", "DTSTAMP:20260801T000000Z",
                "DTSTART:20260810T090000Z", "DTEND:20260810T100000Z",
                "RRULE:FREQ=DAILY;COUNT=5", "SUMMARY:Standup", "END:VEVENT",
                "BEGIN:VEVENT", "UID:rc-1", "RECURRENCE-ID;RANGE=THISANDFUTURE:20260812T090000Z",
                "DTSTAMP:20260801T000000Z",
                "DTSTART:20260812T090000Z", "DTEND:20260812T100000Z",
                "RRULE:FREQ=DAILY;INTERVAL=2;COUNT=2", "SUMMARY:Standup (every other day)", "END:VEVENT",
                "END:VCALENDAR", "");
        ...
        ICalTextOutput built = BuildICalendar.buildICalendar(ax, parsed);
        assertFalse(built.hasError(),
                "a shape ParseICalendar accepts must not be refused by BuildICalendar: "
                        + built.getError());
        String unfolded = built.getIcsText().replace("\r\n ", "");
        assertTrue(unfolded.contains("RECURRENCE-ID"), "RECURRENCE-ID must survive");
        assertTrue(unfolded.contains("INTERVAL=2"), "the override's own RRULE must survive");
    }
